...
bool UBoardPawnMovementComponent::SetMovingDirection(EMovingDirection dir) {
	...
	if (!canMoveOnNewDirection) {
		...
		if (!canMoveOnNewDirection) { UE_LOG(LogTemp, Error, TEXT("SetMovingDirection distance: <%f, %f>, %s"), distanceFromAxis.X, distanceFromAxis.Y, *GetOwner()->GetName()); }
	}
...
FVector UBoardPawnMovementComponent::OnTileCenterRecovery(float deltaTime, FVector delta) {
	...
	if (delta.GetAbsMax() >= PointLikeTolerance * 4.f && !OnTileCenterInfo.IsOnTileCenter) {
		UE_LOG(LogTemp, Warning, TEXT("Last frame-time was too long, %s moved too much and a collision with a central trigger may have been lost. Delta = <%f, %f>"), *GetOwner()->GetName(), delta.X, delta.Y);
